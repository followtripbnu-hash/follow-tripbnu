...
<body class="bg-black text-neutral-100 min-h-screen flex flex-col">
    ...
    <script type="text/babel" data-type="module">
        ...
        // =========================================================================
        // CONFIGURAÇÃO DO FIREBASE (PREENCHA PARA PUBLICAR)
        // =========================================================================
  ...
  measurementId: "G-K0VPC2XLMS"
        ...
        let app, auth, db;
        try {
            // Verifica se a config foi preenchida (não é mais o placeholder)
            ...
            return (
                <div className="font-sans text-neutral-100 flex-grow flex flex-col min-h-screen">
                    ...
                    <main className="max-w-6xl mx-auto p-4 md:p-8 flex-grow w-full relative z-0">
                        ...
                        {!loading && view === 'admin' && adminAuthenticated && <AdminDashboard events={events} appId={appId} db={db} user={user} notify={notify} currentPassword={adminPassword} setAdminPassword={setAdminPassword} />}
        ...
        // --- FLUXO DE RESERVA (COM CONTRATO JURÍDICO) ---
        ...
        function BookingWizard({ event, appId, db, user, onBack, notify }) {
            ...
            // CONTRATO PASSAGEIRO
            const contractTxt = `CONTRATO DE PRESTAÇÃO DE SERVIÇOS DE TRANSPORTE TURÍSTICO\n\nCONTRATADA: FOLLOW TRIP, representada por Izaias Galom dos Santos Junior.\n\nCONTRATANTE: ${formData.name.toUpperCase()}, portador do CPF ${formData.cpf}, RG ${formData.rg} (${formData.issuer || 'SSP'}).\n\nOBJETO: Transporte rodoviário fretado ida e volta para o evento ${event.title.toUpperCase()} em ${event.eventCity.toUpperCase()}.\n\nSAÍDA: ${masks.date(event.departureDate)} às ${event.departureTime}.\n\nCLÁUSULAS GERAIS:\n1. O PASSAGEIRO deve portar documento original com foto no embarque.\n2. É terminantemente proibido o porte e uso de substâncias ilícitas no interior do veículo.\n3. O não comparecimento no horário de embarque (tolerância de 10 minutos) será considerado desistência, sem direito a reembolso.\n4. CANCELAMENTOS: Até 7 dias antes da viagem, reembolso de 80% do valor pago. Após este prazo, não haverá reembolso, sendo permitida apenas a troca de passageiro mediante aviso prévio de 48h.\n5. O passageiro é responsável por seus pertences pessoais.\n\nAo aceitar estes termos digitalmente, o CONTRATANTE declara estar ciente e de acordo com todas as cláusulas acima.`;
        ...
        function AdminDashboard({ events, appId, db, user, notify, currentPassword, setAdminPassword }) {
            ...
            const handleImageUpload = async (e, isEdit = false) => {
                ...
                try {
                    ...
                    if (isEdit) {
                        ...
                        setNewEvent({...newEvent, imageUrl: base64});
                    }
            ...
            const handleUpdate = async () => {
                if(!editingEvent.title) return notify("Título obrigatório", "error");
                try {
                    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'events', editingId), editingEvent);
            ...
            const handleSavePasswords = async () => {
                if(!newAdminPass && !newDriverPass) return notify("Digite uma nova senha.", "error");
                try {
                    if (newAdminPass) {
                        await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'config', 'admin'), { password: newAdminPass });
                        setNewAdminPass('');
                        notify("Senha Admin alterada!", "success");
                    }
                    setShowPasswordModal(false);
                } catch(e) { notify("Erro ao alterar.", "error"); }
            };
                ...
                <div className="grid md:grid-cols-4 gap-8">
                    ...
                    {showPasswordModal && (
                        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4">
                            <div className="glass-panel rounded-3xl p-8 max-w-md w-full animate-enter">
                                <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                                    <h3 className="text-xl font-black text-white uppercase">Alterar Senhas</h3>
                                    <button onClick={() => setShowPasswordModal(false)} className="text-neutral-500 hover:text-white"><X size={20}/></button>
                                </div>
                                <div className="space-y-4 mb-6">
                                    <div>
                                        <label className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2 block">Nova Senha Admin</label>
                                        <input type="password" className="w-full bg-neutral-900/80 border border-neutral-700/50 p-3 rounded-xl text-white placeholder-neutral-700 focus:border-red-600 outline-none text-sm transition-all shadow-inner" placeholder="Digite a nova senha" value={newAdminPass} onChange={e => setNewAdminPass(e.target.value)}/>
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2 block">Senha Motorista (para evento)</label>
                                        <p className="text-[9px] text-neutral-600 mb-2">Você pode alterar a senha de cada motorista ao editar a trip.</p>
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <button onClick={() => setShowPasswordModal(false)} className="flex-1 bg-neutral-800 text-white px-4 py-3 rounded-xl font-black uppercase text-xs hover:bg-neutral-700 transition">Cancelar</button>
                                    <button onClick={handleSavePasswords} className="flex-1 bg-red-600 text-white px-4 py-3 rounded-xl font-black uppercase text-xs hover:bg-red-700 transition">Salvar</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {editingId && editingEvent && (
                        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 overflow-y-auto">
                            <div className="glass-panel rounded-3xl p-8 max-w-2xl w-full my-8 animate-enter">
                                <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                                    <h3 className="text-xl font-black text-white uppercase">Editar Trip</h3>
                                    <button onClick={() => {setEditingId(null); setEditingEvent(null);}} className="text-neutral-500 hover:text-white"><X size={20}/></button>
                                </div>
                                <div className="space-y-4 mb-6 max-h-[60vh] overflow-y-auto">
                                    <div className="md:col-span-2"><AdminInput label="Nome do Evento" value={editingEvent.title} onChange={e => setEditingEvent({...editingEvent, title: e.target.value})}/></div>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <AdminInput label="Cidade Destino" value={editingEvent.eventCity} onChange={e => setEditingEvent({...editingEvent, eventCity: e.target.value})}/>
                                        <AdminInput label="Preço (R$)" type="number" value={editingEvent.price} onChange={e => setEditingEvent({...editingEvent, price: e.target.value})}/>
                                    </div>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <AdminInput label="Data Ida" type="date" value={editingEvent.departureDate} onChange={e => setEditingEvent({...editingEvent, departureDate: e.target.value})}/>
                                        <AdminInput label="Hora Ida" type="time" value={editingEvent.departureTime} onChange={e => setEditingEvent({...editingEvent, departureTime: e.target.value})}/>
                                    </div>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        <AdminInput label="Data Volta" type="date" value={editingEvent.returnDate} onChange={e => setEditingEvent({...editingEvent, returnDate: e.target.value})}/>
                                        <AdminInput label="Hora Volta" type="time" value={editingEvent.returnTime} onChange={e => setEditingEvent({...editingEvent, returnTime: e.target.value})}/>
                                    </div>
                                    <div className="bg-red-900/10 p-4 rounded-xl border border-red-500/20">
                                        <AdminInput label="Senha para Motorista" value={editingEvent.driverPassword} onChange={e => setEditingEvent({...editingEvent, driverPassword: e.target.value})}/>
                                    </div>
                                    <div>
                                        <label className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2 block">Imagem do Evento</label>
                                        <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, true)} className="w-full p-3 border border-neutral-700/50 rounded-xl bg-neutral-900/80 text-neutral-400 cursor-pointer hover:border-red-600 transition-all file:bg-red-600 file:text-white file:border-0 file:px-4 file:py-2 file:rounded-lg file:cursor-pointer file:font-black"/>
                                        {editingEvent.imageUrl && <p className="text-[9px] text-emerald-500 mt-2">✓ Imagem carregada</p>}
                                    </div>
                                </div>
                                <div className="flex gap-3">
                                    <button onClick={() => {setEditingId(null); setEditingEvent(null);}} className="flex-1 bg-neutral-800 text-white px-4 py-3 rounded-xl font-black uppercase text-xs hover:bg-neutral-700 transition">Cancelar</button>
                                    <button onClick={handleUpdate} className="flex-1 bg-white text-black px-4 py-3 rounded-xl font-black uppercase text-xs hover:bg-neutral-200 transition">Atualizar</button>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="glass-panel p-6 rounded-3xl h-fit">
                        ...
                        <nav className="space-y-2">
                            ...
                            <button onClick={() => {setTab('create'); setSelectedId(null); setEditingId(null);}} className={`w-full text-left p-3 rounded-xl font-bold uppercase text-xs tracking-widest transition-all ${tab==='create' ? 'bg-red-600 text-white' : 'text-neutral-500 hover:text-white hover:bg-white/5'}`}>Criar Nova Trip</button>
                            <button onClick={() => setShowPasswordModal(true)} className="w-full text-left p-3 rounded-xl font-bold uppercase text-xs tracking-widest transition-all text-neutral-500 hover:text-white hover:bg-white/5 flex items-center gap-2"><Lock size={14}/> Trocar Senhas</button>
                        </nav>
                    </div>
                    ...
                    <div className="md:col-span-3 glass-panel p-8 rounded-3xl min-h-[500px]">
                        {tab === 'create' && (
                            <div className="space-y-6 animate-enter">
                                <h2 className="text-2xl font-black text-white uppercase italic mb-6">Cadastrar Nova Trip</h2>
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <AdminInput label="Hora Volta" type="time" value={newEvent.returnTime} onChange={e => setNewEvent({...newEvent, returnTime: e.target.value})}/>
                                    ...
                                    <div className="md:col-span-2">
                                        <label className="text-[10px] font-black text-neutral-500 uppercase tracking-widest mb-2 block">Upload da Imagem</label>
                                        <input type="file" accept="image/*" onChange={handleImageUpload} className="w-full p-4 border border-neutral-700/50 rounded-xl bg-neutral-900/80 text-neutral-400 cursor-pointer hover:border-red-600 transition-all file:bg-red-600 file:text-white file:border-0 file:px-4 file:py-2 file:rounded-lg
